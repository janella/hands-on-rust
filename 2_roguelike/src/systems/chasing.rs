...
pub fn chasing(#[resource] map: &Map, ecs: &SubWorld, commands: &mut CommandBuffer) {
    ...
    let (player_pos, _) = player.iter(ecs).next().unwrap();
    let player_idx = get_map_idx(player_pos.x, player_pos.y);

    let search_targets = vec![player_idx];
    let dijsktra_map = DijkstraMap::new(SCREEN_WIDTH, SCREEN_HEIGHT, &search_targets, map, 1024.0);
    ...
    movers.iter(ecs).for_each(|(entity, pos, _, fov)| {
        ...
        if let Some(destination) = DijkstraMap::find_lowest_exit(&dijsktra_map, idx, map) {
